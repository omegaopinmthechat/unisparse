...
%%%%                                                                   %%%%
%%%%  This script runs ALL uni-sparse methods:                        %%%%
%%%%    - UniLASSO (L1 penalty)                                       %%%%
%%%%    - UniMCP   (MCP penalty)                                      %%%%
%%%%    - UniSCAD  (SCAD penalty)                                     %%%%
%%%%                                                                   %%%%
%%%%  Plus baseline comparison methods:                               %%%%
%%%%    - LASSO, Adaptive LASSO, SCAD, MCP                            %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% ---------------- SCAD PARAMETER ----------------
a_scad = 3.7;  % SCAD concavity parameter (Fan & Li 2001 default)

% ---------------- MCP PARAMETER ----------------
gamma_mcp = 3.0;  % MCP concavity parameter (Zhang's default)
...
best_test_mse_uniscad  = inf;
best_lambda_uniscad    = NaN;

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%                    CV LOOP  (UNILASSO)                            %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

fprintf('========== Starting UniLASSO Cross-Validation ==========\n');
for il = 1:nl
...
fprintf('\n========== Starting UniSCAD Cross-Validation ==========\n');
for il = 1:nl
    lambda = lambda_grid(il);
    ...
    for f = 1:nfolds
        fprintf('UniSCAD CV | lambda = %.2e | fold %d/%d\n',...
            lambda, f, nfolds);
        ...
        % UniSCAD objective with SCAD parameter a
        objFun = @(psi) uniSCAD_objective_given_eta_loo( ...
            psi, eta_loo, ytr, lambda, a_scad);
    ...
    if test_mse < best_test_mse_uniscad
        best_test_mse_uniscad = test_mse;
        best_lambda_uniscad   = lambda;
    end
end
...
%%%%                     UniSCAD FINAL REFIT                          %%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf('\nRefitting UNISCAD on full data (lambda=%.2e, a=%.1f)\n', ...
        best_lambda_uniscad, a_scad);
...
% ---- 3. Objective function with SCAD penalty ----
objFun_SCAD = @(psi) uniSCAD_objective_given_eta_loo( ...
                       psi, eta_loo, y, best_lambda_uniscad, a_scad);

% ---- 4. Optimize ----
[x_opt_SCAD, ~, ~] = RMPSH(objFun_SCAD, psi0, lb, ub, options);

theta0_hat_SCAD = x_opt_SCAD(1);
theta_hat_SCAD  = x_opt_SCAD(2:end);

% ---- 5. Recover final UniSCAD coefficients ----
...
% ---- 6. Predictions & metrics ----
yhat_full_uniscad = gamma0_hat_SCAD + X * gamma_hat_SCAD;

metrics_row_uniscad = compute_sparse_metrics( ...
    beta_hat_whole_uniscad, beta_true_whole, ...
    yhat_full_uniscad, y, tol);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Method = ["UniLASSO";"UniMCP";"UniSCAD";"LASSO";"AdapLASSO";"SCAD";"MCP"];
...
Results_All = table(Method, Lambda, Gamma_MCP, A_SCAD, TPR, FPR, FDR, MCC, ...
