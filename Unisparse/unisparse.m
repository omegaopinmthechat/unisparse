function results = unisparse(X, y, lambda_grid, nfolds, method, x0, rmps_lb, rmps_ub, rmps_options, a, gamma)
% UNISPARSE_CV  Cross-validated UniSparse estimators (UniLASSO / UniMCP / UniSCAD)
%
% Usage:
%   results = unisparse(X, y, lambda_grid)
...
%   y             - n x 1 response
%   lambda_grid   - either a vector of lambda values or two-element [min,max]
%                   (logspace with 10 points is generated from [min,max])
%   nfolds        - number of CV folds (default: 2)
%   method        - 'unilasso', 'unimcp', 'uniscad' or 'all' (default: 'all')
%   x0            - RMPS starting point (default: [mean(b0); ones(p,1)])
%   rmps_lb       - RMPS lower bounds vector (default: 1e-4 for intercept, 0 for slopes)
%   rmps_ub       - RMPS upper bounds vector (default: 1e4 for all)
...
if nargin < 4  || isempty(nfolds),       nfolds = 2;                 end
if nargin < 5  || isempty(method),       method = 'all';             end
if nargin < 6,                           x0 = [];                    end
if nargin < 7,                           rmps_lb = [];               end
if nargin < 8,                           rmps_ub = [];               end
if nargin < 9,                           rmps_options = [];          end
if nargin < 10 || isempty(a),            a = 3.7;                    end
if nargin < 11 || isempty(gamma),        gamma = 3.0;                end

% --- Prepare lambda grid ---
if ~iscell(lambda_grid)
    if numel(lambda_grid) == 2
        lambda_grid = logspace(log10(lambda_grid(1)), log10(lambda_grid(2)), 10);
    else
...
% Ensure helper folders are on the path
try
...
ub_lb_factor = 1e4;
if isempty(rmps_lb), rmps_lb = [1e-4; zeros(p,1)];              end
if isempty(rmps_ub), rmps_ub = ub_lb_factor * ones(p+1,1);      end
...
% parfor gracefully falls back to serial execution if the Parallel Computing
% Toolbox is unavailable, so no explicit license check is needed.
try
    if isempty(gcp('nocreate'))
        parpool('Processes');
    end
catch
    % non-fatal: parfor will run serially
end
...
% --- Detect parameter sweep ---
sweep_flag = false;
if iscell(method)      && numel(method) > 1,       sweep_flag = true; end
if iscell(lambda_grid) && numel(lambda_grid) > 1,  sweep_flag = true; end
if numel(nfolds) > 1,                              sweep_flag = true; end
if numel(a) > 1,                                   sweep_flag = true; end
if numel(gamma) > 1,                               sweep_flag = true; end
if iscell(rmps_lb)     && numel(rmps_lb) > 1,      sweep_flag = true; end
if iscell(x0)          && numel(x0) > 1,           sweep_flag = true; end

if sweep_flag
    if iscell(method),       methods_list   = method;        else, methods_list   = {method};        end
    if iscell(lambda_grid),  lambda_list    = lambda_grid;   else, lambda_list    = {lambda_grid};   end
    ...
    if iscell(x0),           x0_list        = x0;            else, x0_list        = {x0};            end
    nfolds_list = nfolds(:)';
    a_list      = a(:)';
    gamma_list  = gamma(:)';
    ...
    [IM,IL,IF,IA,IG,ILB,IUB,IX0] = ndgrid( ...
        ...
        1:numel(rmps_ub_list), 1:numel(x0_list));
    ...
    for run_idx = 1:total_runs
        ...
        try
            out = unisparse_cv_single(X, y, cur_lambda, cur_nfolds, cur_method, ...
                      cur_x0, cur_rmps_lb, cur_rmps_ub, rmps_options, cur_a, cur_gamma);
        ...
        results_all(run_idx).params = struct('method', cur_method, 'lambda_grid', cur_lambda, ...
            'nfolds', cur_nfolds, 'a', cur_a, 'gamma', cur_gamma, ...
            'rmps_lb', cur_rmps_lb, 'x0', cur_x0);
...
% --- Single-run path: precompute data splits ---
data = split_data(X, y, nfolds);

% --- CV helper: sweep lambda grid for a given objective builder ---
% parfor runs in parallel when a pool is active, serially otherwise —
% no explicit toolbox check required.
function best_lambda = cv_for_objective(obj_fun_builder)
    ...
    for f = 1:nfolds
        ...
        [b0_fold, b_fold, ~, ~, eta_loo_fold] = unisparse_univreg(X(train_idx,:), y(train_idx));
        B_mat(:,f)  = b_fold(:);
        B0_mat(:,f) = b0_fold(:);
        ETA{f}      = eta_loo_fold;
        PSI0{f}     = [mean(b0_fold); ones(p,1)];
        if ~isempty(x0), PSI0{f} = x0; end
    end
...
% --- UniLASSO CV and final fit ---
if run_unilasso
    fprintf('========== UniLASSO CV (nfolds=%d) ==========\n', nfolds);
    best_lambda_unilasso = cv_for_objective( ...
        @(psi0,eta_loo,ytr,lambda) (@(psi) unilasso_objective_given_eta_loo(psi, eta_loo, ytr, lambda)));

    [b0, b, ~, ~, eta_loo] = unisparse_univreg(X, y);
    psi0 = [mean(b0); ones(p,1)];
    if ~isempty(x0), psi0 = x0; end

    [x_opt_L1,~,~] = RMPSH(@(psi) unilasso_objective_given_eta_loo(psi, eta_loo, y, best_lambda_unilasso), ...
                             psi0, rmps_lb, rmps_ub, rmps_options);
    theta0_L1 = x_opt_L1(1);  theta_L1 = x_opt_L1(2:end);
    gamma0_L1 = theta0_L1 + sum(b0(:) .* theta_L1(:));
    gamma_L1  = b(:) .* theta_L1(:);

    results.UNILASSO.lambda       = best_lambda_unilasso;
    results.UNILASSO.beta         = [gamma0_L1; gamma_L1];
    results.UNILASSO.rmps_options = rmps_options;
end

% --- UniMCP CV and final fit ---
if run_unimcp
    fprintf('========== UniMCP CV (nfolds=%d) ==========\n', nfolds);
    best_lambda_unimcp = cv_for_objective( ...
        @(psi0,eta_loo,ytr,lambda) (@(psi) uniMCP_objective_given_eta_loo(psi, eta_loo, ytr, lambda, gamma)));

    [b0, b, ~, ~, eta_loo] = unisparse_univreg(X, y);
    psi0 = [mean(b0); ones(p,1)];
    if ~isempty(x0), psi0 = x0; end

    [x_opt_MCP,~,~] = RMPSH(@(psi) uniMCP_objective_given_eta_loo(psi, eta_loo, y, best_lambda_unimcp, gamma), ...
                              psi0, rmps_lb, rmps_ub, rmps_options);
    theta0_MCP = x_opt_MCP(1);  theta_MCP = x_opt_MCP(2:end);
    gamma0_MCP = theta0_MCP + sum(b0(:) .* theta_MCP(:));
    gamma_MCP  = b(:) .* theta_MCP(:);

    results.UNIMCP.lambda       = best_lambda_unimcp;
    results.UNIMCP.beta         = [gamma0_MCP; gamma_MCP];
    results.UNIMCP.gamma        = gamma;
    results.UNIMCP.rmps_options = rmps_options;
end

% --- UniSCAD CV and final fit ---
if run_uniscad
    fprintf('========== UniSCAD CV (nfolds=%d) ==========\n', nfolds);
    best_lambda_uniscad = cv_for_objective( ...
        @(psi0,eta_loo,ytr,lambda) (@(psi) uniSCAD_objective_given_eta_loo(psi, eta_loo, ytr, lambda, a)));

    [b0, b, ~, ~, eta_loo] = unisparse_univreg(X, y);
    psi0 = [mean(b0); ones(p,1)];
    if ~isempty(x0), psi0 = x0; end

    [x_opt_SCAD,~,~] = RMPSH(@(psi) uniSCAD_objective_given_eta_loo(psi, eta_loo, y, best_lambda_uniscad, a), ...
                               psi0, rmps_lb, rmps_ub, rmps_options);
    theta0_SCAD = x_opt_SCAD(1);  theta_SCAD = x_opt_SCAD(2:end);
    gamma0_SCAD = theta0_SCAD + sum(b0(:) .* theta_SCAD(:));
    gamma_SCAD  = b(:) .* theta_SCAD(:);

    results.UNISCAD.lambda       = best_lambda_uniscad;
    results.UNISCAD.beta         = [gamma0_SCAD; gamma_SCAD];
    results.UNISCAD.a            = a;
...
% =========================================================================
function out = unisparse_cv_single(X, y, lambda_grid, nfolds, method, x0, rmps_lb, rmps_ub, rmps_options, a, gamma)
% Runs a single configuration (no sweeping) and returns a results struct.
    ...
    if isempty(rmps_lb), rmps_lb = [1e-4; zeros(ploc,1)];             end
    if isempty(rmps_ub), rmps_ub = ub_lb_factor * ones(ploc+1,1);     end
    if isempty(rmps_options)
        rmps_options.DisplayUpdate = 0;
        rmps_options.PrintSolution = 0;
        rmps_options.MaxRuns       = 5;
    ...
    if run_unil
        ...
        [x_opt_L1,~,~] = RMPSH(@(psi) unilasso_objective_given_eta_loo(psi, eta_loo, y, best_lambda_unil), ...
                                 psi0, rmps_lb, rmps_ub, rmps_options);
        theta0_L1 = x_opt_L1(1);  theta_L1 = x_opt_L1(2:end);
        gamma0_L1 = theta0_L1 + sum(b0(:) .* theta_L1(:));
        gamma_L1  = b(:) .* theta_L1(:);
        ...
        results_local.UNILASSO.beta   = [gamma0_L1; gamma_L1];
    end

    % --- UniMCP ---
    if run_mcp
        best_lambda_mcp = cv_run_local( ...
            @(psi0,eta_loo,ytr,lambda) (@(psi) uniMCP_objective_given_eta_loo(psi, eta_loo, ytr, lambda, gamma)));
        ...
        [x_opt_MCP,~,~] = RMPSH(@(psi) uniMCP_objective_given_eta_loo(psi, eta_loo, y, best_lambda_mcp, gamma), ...
                                  psi0, rmps_lb, rmps_ub, rmps_options);
        theta0_MCP = x_opt_MCP(1);  theta_MCP = x_opt_MCP(2:end);
        gamma0_MCP = theta0_MCP + sum(b0(:) .* theta_MCP(:));
        gamma_MCP  = b(:) .* theta_MCP(:);
        ...
        results_local.UNIMCP.beta   = [gamma0_MCP; gamma_MCP];
        results_local.UNIMCP.gamma  = gamma;
    end

    % --- UniSCAD ---
    if run_scad
        best_lambda_scad = cv_run_local( ...
            @(psi0,eta_loo,ytr,lambda) (@(psi) uniSCAD_objective_given_eta_loo(psi, eta_loo, ytr, lambda, a)));
        ...
        [x_opt_SCAD,~,~] = RMPSH(@(psi) uniSCAD_objective_given_eta_loo(psi, eta_loo, y, best_lambda_scad, a), ...
                                   psi0, rmps_lb, rmps_ub, rmps_options);
        theta0_SCAD = x_opt_SCAD(1);  theta_SCAD = x_opt_SCAD(2:end);
        gamma0_SCAD = theta0_SCAD + sum(b0(:) .* theta_SCAD(:));
        gamma_SCAD  = b(:) .* theta_SCAD(:);
        ...
        results_local.UNISCAD.beta   = [gamma0_SCAD; gamma_SCAD];
